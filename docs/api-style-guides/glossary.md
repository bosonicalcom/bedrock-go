...
The entity that consumes an API service. For Bosonical APIs, it typically is a
Bosonical project that owns the client application or the server resource.
...
The entity that produces an API service. For Bosonical APIs, it typically is a
Bosonical team responsible for the API service.
...
Bosonical API service definition is `google.api.Service`.
...
Refers to the logical identifier of an API service. Bosonical APIs use RFC 1035 DNS
...
A Network API exposed by a Bosonical service. Most of these are hosted on the
