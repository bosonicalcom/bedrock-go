...
This topic describes the versioning strategies used by Bosonical APIs. In
general, these strategies apply to all Bosonical-managed services.

## Guidance

All Bosonical API interfaces **must** provide a _major version number_, which is
...
Bosonical APIs **must not** expose minor or patch version numbers. For
example, Bosonical APIs use `v1`, not `v1.0`, `v1.1`, or `v1.4.2`. From a user's
...
[API visibility][] is an advanced feature provided by Bosonical API infrastructure.
...
Bosonical APIs often use API visibility for Preview features.
